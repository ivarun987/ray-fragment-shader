...
void drawSphere(inout Pixel pixel, inout Ray ray, in Sphere sphere, in Light light) {
    ...
    if (ray_length > epsilon && ray_length < infini) {
        pixel.color = computePhongShading(ray, intersection, sphere.material, light);
    }
}

void drawPlane(inout Pixel pixel, inout Ray ray, in Plane plane, in Light light) {
    Ray intersection = ray;
    float ray_length = computePlaneIntersection(intersection, plane);
    if (ray_length > epsilon && ray_length < infini) {
        pixel.color = computePhongShading(ray, intersection, computeCheckboard(intersection.origin, plane), light);
    }
}
...
void main(){
    ...
    drawPlane(pixel, ray, plane, light);
