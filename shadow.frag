...
void drawScene(inout Pixel pixel, inout Ray ray, in Sphere sphere, in Plane plane, in Light light) {
    ...
    if (id == sphereId) {
        ...
        pixel.color = computePhongShading(ray, intersection, sphere.material, light, shadow_factor);
    } else if (id == planeId) {
        float shadow_factor = computeShadowFactor(intersection, plane.material, sphere, plane, light);
        pixel.color = computePhongShading(ray, intersection, computeCheckboard(intersection.origin, plane), light, shadow_factor);
    }
}
